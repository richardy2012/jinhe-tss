		...
		<xsl:choose>
			...
			<xsl:when expr="getMode()=='string' &amp;&amp; (getProperty('editor')=='textarea' || this.tagName=='textarea' || this.getAttribute('type')=='textarea')">
				<textarea>
					...
					<xsl:attribute name="value"><xsl:eval>getValue(this.getAttribute("binding"))</xsl:eval></xsl:attribute>					<xsl:eval>getValue(this.getAttribute("binding"))</xsl:eval></textarea>
			</xsl:when>
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when expr="this.tagName=='xmlIsland'">
						<xml>
							<xsl:apply-templates select="@*" />
							<xsl:if expr="getEditable()=='false'"><xsl:attribute name="disabled">true</xsl:attribute></xsl:if>
							<xsl:apply-templates/>
						</xml>
					</xsl:when>
					<xsl:otherwise>
						<xsl:copy>
							<xsl:apply-templates select="@*" />
							<xsl:if expr="getEditable()=='false'"><xsl:attribute name="disabled">true</xsl:attribute></xsl:if><xsl:apply-templates/>
						</xsl:copy>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
