...
**[它山石](http://waitwind.github.io/)（简称TSS）是一个开源的应用基础平台，专注于提供高效开发各类应用软件所必需的技术框架及常用功能组件，目标是打造成既能支持组件化的开发，又能对既有异构系统提供有效的整合，并对业务建模、开发、整合、部署、业务数据分析提供一致性的工具与平台。在它山石框架和组件的基础上，应用开发者可以直接专注于业务逻辑本身的开发，减轻开发工作量，同时还能把不同的应用无缝衔接集成，自动实现统一用户管理和资源授权、单点登陆（SSO）、数据集中分析、内容聚合展示、应用集成一体。 可快速完成业务系统开发及整合（应用整合、数据整合）目前框架包含的常用功能组件有：
...
主要开发语言为Java、JavaScript，数据库支持MySQL、H2、Oracle。其中后端框架底层采用JPA2.0 + Spring，前端为HTML5 + JS + CSS3，前后台交互使用AJAX。

- 前端框架采用独立的tssJS框架，提供常用DOM操作、事件处理、AJAX交互、XML数据处理等核心封装、统一的界面样式布局，及菜单、Tree、Grid、Form等常用组件。
- 前后端交换一律采用AJAX，数据格式同时支持XML和JSON 
- 使用Freemarker作为模板解析引擎，应用于门户、数据图表等支持模板化二次开发的模块。
- Spring MVC将后端服务发布成web服务，使用restful风格来。
- Spring提供IOC容器及AOP机制，对业务操作对象的生成、依赖注入、事务、权限过滤、日志搜集、缓存等进行统一管理。
- 持久层采用JPA2.0，实现对业务对象进行持久化和常用查询。支持配置自定义的数据源，可同时对多数据源进行访问操作。

> **Note:**

> - 较少采用各类流行但体量庞大的框架，更多使用了原创的替代解决方案，精致实用，易扩展、易维护。 
> - 前后端充分解耦合，开发时各自独立进行、互不依赖，只需彼此约定好数据格式，前端直接依据模拟数据文件进行开发，待后端开发好正式的接口服务，前端将<b>数据服务地址</b>由模拟文件替换为后端正式的服务地址。
